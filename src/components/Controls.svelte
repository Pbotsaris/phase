...
<select class="select" bind:value={selectedTonality} on:change={() => handleTonalityChange(scale)}>
  {#each scale.tonalities as tonality}
    <option value={tonality}> {tonality} </option>
  {/each}
</select>

<select class="select" bind:value={selectedKey} on:change={() => handleKeyChange(scale)}>
